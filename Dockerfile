...
# Installa le dipendenze del progetto (salta l'installazione del progetto stesso per ora)
RUN uv sync --frozen --no-install-project --no-dev

# Copia il resto del codice della tua applicazione
COPY app ./app/

# Comando per avviare la tua app NiceGUI (es. main.py)
CMD ["uv", "run", "app/run_web.py"]

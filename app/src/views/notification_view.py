...
def NotificationView():
	...
	with ui.card():
		...
		with ui.card_actions():
			ui.button(
				'Invia Info',
				on_click=lambda: logger.info(f'Un messaggio informativo da {NAME}:{ui.context.client.id}'),
			)
			ui.button(
				'Invia Errore',
				on_click=lambda: logger.error(f'Qualcosa è andato storto! da {NAME}:{ui.context.client.id}'),
			)

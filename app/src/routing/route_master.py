...
class MasterRoute:
	_ROUTES: TypedRoutes = {
		...
		'/login': {'label': 'Login', 'icon': 'login', 'component': LoginView, 'childrens': []},
		'/register': {'label': 'Register', 'icon': 'register', 'component': RegisterView, 'childrens': []},
	}
	...
	def setup(cls, path_initial: str):
		...
		ui.context.client.sub_pages_router.on_path_changed(lambda path: cls.handle_path_change(path))
	...
	def get_router_links(cls):
		selected_data = [
			{'path': path, 'label': data['label'], 'icon': data['icon']} for path, data in cls.ROUTES.items()
		]
